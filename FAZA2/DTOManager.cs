        ...
        public static List<DetePregled> VratiSvuDecu()
        {
            try
            {
                using (var s = DataLayer.GetSession())
                {
                    ...
                    return listaDecaPregled;
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message);
                return new List<DetePregled>();
            }
        }

        public static Dete AzurirajDete(int id, string ime, string prezime, DateTime datumRodjenja, char pol,
            string adresa, string telefonDeteta, string emailDeteta, string posebnePotrebe)
        {
            try
            {
                using (var s = DataLayer.GetSession())
                {
                    ...
                    dete.Ime = string.IsNullOrEmpty(ime) ? dete.Ime : ime;
                    dete.Prezime = string.IsNullOrEmpty(prezime) ? dete.Prezime : prezime;
                    dete.DatumRodjenja = datumRodjenja != DateTime.MinValue ? datumRodjenja : dete.DatumRodjenja;
                    dete.Pol = pol != default ? pol : dete.Pol;
                    dete.Adresa = string.IsNullOrEmpty(adresa) ? dete.Adresa : adresa;
                    dete.TelefonDeteta = string.IsNullOrEmpty(telefonDeteta) ? dete.TelefonDeteta : telefonDeteta;
                    dete.EmailDeteta = string.IsNullOrEmpty(emailDeteta) ? dete.EmailDeteta : emailDeteta;
                    dete.PosebnePotrebe = string.IsNullOrEmpty(posebnePotrebe) ? dete.PosebnePotrebe : posebnePotrebe;
